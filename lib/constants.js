...
${serpContext ? `REAL-TIME SERP DATA (FOCUS ON 2025-2026 RESULTS ONLY):\n${serpContext}\n\n` : ""}

For each competitor, analyze based strictly on futuristic/emerging (2025+) patterns:
- **FUTURE Content Patterns** — isolate their newest successful formats.
- **Linked-To Content** — identify their newest link magnets.
- **EMERGING CONTENT GAPS** — identify 3 specific, high-intent gaps they have ignored for 2025/2026.

Provide:
- 📊 **COMPARATIVE METRICS TABLE**: Competitor | Content Depth | Key Advantage | Recent Gap
- **STRATEGIC COMMAND** — one move to outrank their newest content.
...
export const PROMPT_STEP4 = (topic, serpContext = "") => {
  const contextBlock = serpContext 
    ? `REAL-TIME SERP DATA Context:\n${serpContext}\nUse this live search data to accurately inform keyword difficulty, search intent, and identify realistic long-tail targets.\n\n` 
    : "";

  return `Perform deep Keyword Research for the topic: "${topic}".

${contextBlock}Focus ONLY on keywords that are trending or gaining traction in 2025-2026. Ignore keywords that peaked before 2024.
Analyze high-intent, long-tail opportunities based on achievable positioning for this exact topic.
